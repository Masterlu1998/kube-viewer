...
func NewScrapperManagement(ctx context.Context, collector *debug.DebugCollector) (*ScrapperManagement, error) {
	...
	sMap := map[string]Scrapper{
		workload.DeploymentScrapperTypes:     workload.NewDeploymentListScrapper(kubeLister, kubeClient, collector),
		workload.StatefulSetScrapperTypes:    workload.NewStatefulSetListScrapper(kubeLister, kubeClient, collector),
		workload.DaemonSetScrapperTypes:      workload.NewDaemonSetListScrapper(kubeLister, kubeClient, collector),
		workload.ReplicaSetScrapperTypes:     workload.NewReplicaSetListScrapper(kubeLister, kubeClient, collector),
		workload.CronJobScrapperTypes:        workload.NewCronJobListScrapper(kubeLister, kubeClient, collector),
		workload.JobScrapperTypes:            workload.NewJobListScrapper(kubeLister, kubeClient, collector),
		service.ServiceScrapperTypes:         service.NewNServiceListScrapper(kubeLister, kubeClient, collector),
